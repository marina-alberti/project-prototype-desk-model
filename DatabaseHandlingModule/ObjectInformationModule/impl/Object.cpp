...
bool isContainedString(string container, string small) {

	return (container.find(small) != std::string::npos);

}

void Object::setObjectNameLargeDataset(string inputName) {

	  objectName = inputName;

	  // set "actualObjectID" and "categoryName" fields based on the name

	  if ( isContainedString(inputName, "Monitor") ) {
	    actualObjectID = 0;
	    categoryName = "Monitor";
	  }

	  else if ( isContainedString(inputName, "Keyboard") ){
		  actualObjectID = 1;
		  categoryName = "Keyboard";
	  }

	  else if ( isContainedString(inputName, "Mouse") ) {
		  actualObjectID = 2;
		  categoryName = "Mouse";		}

	  else if ( isContainedString(inputName, "Mug") ) {
		  actualObjectID = 3;
		  categoryName = "Mug";
	  }

	  else if ( isContainedString(inputName, "Lamp") ) {
		  actualObjectID = 4;
		  categoryName = "Lamp";
	  }

	  else if ( isContainedString(inputName, "Notebook") ) {
		  actualObjectID = 5;
		  categoryName = "Notebook";

	  }
	  else if ( isContainedString(inputName, "Laptop") ) {
		  actualObjectID = 6;
		  categoryName = "Laptop";

	  }
	  else if ( isContainedString(inputName, "Papers") ) {
		  actualObjectID = 7;
		  categoryName = "Papers";

	  }
	  else if ( isContainedString(inputName, "Book") ) {
		  actualObjectID = 8;
		  categoryName = "Book";

	  }
	  else if ( isContainedString(inputName, "Mobile") ) {
		  actualObjectID = 9;
		  categoryName = "Mobile";
		...
		//else if (strcmp(nameChar, "Flask")== 0 ) {
			//intLabel = 10;
		//}
	  else if ( isContainedString(inputName, "Glass") ) {
		  actualObjectID = 10;
		  categoryName = "Glass";

	  }
	  else if ( isContainedString(inputName, "Jug") ) {
		  actualObjectID = 11;
		  categoryName = "Jug";

	  }
	  else if ( isContainedString(inputName, "Headphones") ) {
		  actualObjectID = 12;
		  categoryName = "Headphones";

	  }
	  else if ( isContainedString(inputName, "Bottle") ) {
		  actualObjectID = 13;
		  categoryName = "Bottle";
	}
